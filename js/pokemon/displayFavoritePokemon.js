...
const displayFavoritePokemon = async () => {
  try {
    ...
    let existingFavoritePokemonIds = JSON.parse(localStorage.getItem("favoritePokemonIds")) || [];
    ...
    localStorage.setItem("favoritePokemonIds", JSON.stringify(existingFavoritePokemonIds));

    //Fetch data for each saved pokemonID and display them after all have been fetched
    const pokemonPromises = existingFavoritePokemonIds.map(async (pokemonId) => {
      try {
